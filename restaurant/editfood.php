    ...
    <div class="login-card">
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="name" class="form-control" placeholder="กรอกชื่อเมนูของคุณ"  value="<?=$row['name'];?>"required>
            </div>
            <div class="mb-3">
                <label for="price" class="form-label">ราคา</label>
                <input type="number" name="price" class="form-control" placeholder="กรอกราคาของคุณ"  value="<?=$row['price'];?>"required>
            </div>
            <div class="mb-3">
...
if (isset($_POST['submit'])) {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        if (in_array($ext, ['jfif', 'jpg', 'jpeg', 'png'])) {
            ...
            $stmt = $conn->prepare('UPDATE product SET res_id = ? , category_id  = ? ,name  =? , image = ? , price = ? , discount  = ? ,discounted_price= ? WHERE id =?  ');
            $result = $stmt->execute([$res_id, $category, $name, $newfilename, $price, $discount,$discounted_price,$id]);
