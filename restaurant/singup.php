...
<body>
    <div class="login-card">
        <h3 class="text-center mb-4">สมัครเข้าใช้ร้านอาหาร</h3>
        <form  method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input type="text"  name="name" class="form-control" placeholder="กรอกชื่อร้านของคุณ" required>
            </div>
            <div class="mb-3">
...
if(isset($_POST['submit'])){
 ...
 $email =$_POST['email'];
 $pass =$_POST['pass'];
 if(empty($name) || empty($phone) || empty($address) || empty($pass)){
   $_SESSION['error'] ='error';
    ...
    if($stmt->rowCount() > 0){
        ...
        if(in_array($ext ,['jfif','jpg','jpeg','png'])){
            ...
            $stmt =$conn->prepare('INSERT INTO restaurant(name,image,phone,address,email,password) VALUES(?,?,?,?,?,?)');
            $result = $stmt->execute([$name,$newfilename,$phone,$address,$email,$pass]);
            if($result){
