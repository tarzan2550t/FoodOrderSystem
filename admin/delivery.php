...
<body>
    ...
    <div class="d-flex mb-4">
      <a href="adddeli.php" class="btn btn-danger">เพิ่มผู้ส่งอาหาร</a>
    </div>
        <div class="card  text-center">
            <div class="card-header bg-dark text-white">
                <h5 class="mb-0">ผู้ส่งอาหาร</h5>
            </div>
            <div class="card-body">
                <table class="table table-striped">
                    ...
                    <?php 
                    $stmt =$conn->prepare('SELECT delivery.*,status.name as status_name , status.id as status_id FROM delivery LEFT JOIN status ON delivery.status = status.id ');
                    $stmt->execute();
                    ...
                    <tr>
                        ...
                        <td><a href="editdeli.php?id=<?=$row['id'];?>" class="btn btn-success btn-sm">แก้ไขข้อมูล</a></td>
                        <td><a href="deletedeli.php?id=<?=$row['id'];?>" class="btn btn-danger btn-sm" onclick="return confirm('ยืนยันการลบบัญชีผู็ใช้!!');">ลบข้อมูล</a></td>
                        <td><a href="statusdeli.php?id=<?=$row['id'];?>" class="btn btn-success btn-sm"onclick="return confirm('ยืนยันการอนุญาตผู้ใช้งาน!!');">อนุญาต</a></td>
                        <td><a href="notstatusdeli.php?id=<?=$row['id'];?>" class="btn btn-danger btn-sm"onclick="return confirm('ยืนยันการะงับผู้ใช้งาน!!');">ระงับผู้ใช้งาน</a></td>
                    </tr>
                    <?php }?>
                 </table>
               </div>
